 ...
 *     in two steps.
 *  9. Opening a market and settling one are two different roles. This is the
 *     only change from V3, and it exists so that market creation can be
 *     automated without the automating key being able to decide an outcome.
 *
 * On the split in property 9. V3 put registerPrediction and resolvePrediction
 * behind one onlyResolver check. Anything registering markets on a server
 * therefore held a key that could also declare who won, and declaring a winner
 * moves the losing pool. A registrar here can bring a market into existence and
 * nothing else: it cannot resolve, cancel, refund, change a fee, or touch a
 * balance. The worst a stolen registrar key can do is register junk markets,
 * which cost the thief nothing to make and earn them nothing. That is a spam
 * problem, and spam is survivable in a way that a forged settlement is not.
 *
 ...
 * amount.
 */
contract PredictionMarket_V4 is Ownable2Step, ReentrancyGuard {
    ...
    /// @notice Addresses allowed to open markets, and to do nothing else.
    ///         Deliberately not the same set as resolvers: this is the key a
    ///         server can hold.
    ...
    event RegistrarSet(address indexed registrar, bool enabled);
    ...
    /**
     * @dev Guards market creation, and nothing else in this contract uses it.
     *
     * A resolver is deliberately NOT accepted here. Letting the settling role
     * register as well would put the two back in one key the moment anyone
     * reused their resolver for automation, which is exactly the shape V3 had.
     * The owner passes because the owner can grant itself the role anyway, so
     * refusing it would only add a transaction.
     */
    modifier onlyRegistrar() {
        require(msg.sender == owner() || registrars[msg.sender], "Not a registrar");
        _;
    }
    ...
    /**
     * @notice Grant or revoke the right to open markets.
     * @dev Revocable and cheap, which is the point: this is the key that lives
     *      on a server, so it has to be replaceable without touching ownership
     *      or interrupting any market already running.
     */
    function setRegistrar(address registrar, bool enabled) external onlyOwner {
        ...
        emit RegistrarSet(registrar, enabled);
    }
