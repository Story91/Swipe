...
/**
 * @title SwipeDailyRewards V2 - Improved daily task rewards system
 * @dev V2 improvements:
 * - Blacklist support to prevent abuse
 * - Editable reward amounts (admin configurable)
 * - Signature verification for referrals (prevents farming)
 * - Reduced referral rewards
 * - Data migration from V1 contract
 *
 * Pool: 250M SWIPE
 *
 * Reward Structure (configurable):
 * - Base daily claim: 50,000 SWIPE (default)
 ...
 * - Invite friend: +50,000 SWIPE each (reduced from 150k)
 *
...
contract SwipeDailyRewards_V2 {
    ...
    uint256 public referralReward = 50_000 * 10**18;               // Invite friend (reduced from 150k)
    ...
    bool public migrationEnabled = true; // Allow migration from V1

    // Blacklist
    mapping(address => bool) public blacklist;

    // User data structure (same as V1 for compatibility)
    struct UserData {
        uint256 lastClaimTimestamp;
        uint256 currentStreak;
        uint256 longestStreak;
        ...
        bool hasStreak7Achievement;
        bool hasStreak30Achievement;
        bool migrated; // Track if user data was migrated from V1
    }
     ...
     * @param _v1Contract Address of V1 contract for migration
     */
    constructor(
        address _swipeToken,
        ...
        require(_v1Contract != address(0), "Invalid V1 contract address");
    ...
    /**
     * @dev Migrate user data from V1 contract
    ...
    function migrateFromV1() external {
        ...
        // Only migrate if user exists in V1
        ...
        // Update counters if first time in V2
    ...
    /**
     * @dev Batch migrate multiple users (admin only)
     * @param usersToMigrate Array of user addresses to migrate
     */
    function batchMigrateFromV1(address[] calldata usersToMigrate) external onlyOwner {
        require(migrationEnabled, "Migration disabled");
        ...
        for (uint256 i = 0; i < usersToMigrate.length; i++) {
            ...
            // Migrate referral data
            address v1Referrer = v1Contract.referredBy(userAddr);
            if (v1Referrer != address(0)) {
                referredBy[userAddr] = v1Referrer;
                hasUsedReferral[userAddr] = v1Contract.hasUsedReferral(userAddr);
            }
     ...
     */
    function claimDaily() external whenNotPaused whenClaimingEnabled notBlacklisted(msg.sender) {
        UserData storage user = users[msg.sender];
    ...
    function _verifyReferralSignature(
        ...
        // Create message hash for referral
        bytes32 messageHash = keccak256(
    ...
    function batchSetBlacklist(address[] calldata users, bool[] calldata blacklisted) external onlyOwner {
        require(users.length == blacklisted.length, "Array length mismatch");
        for (uint256 i = 0; i < users.length; i++) {
            blacklist[users[i]] = blacklisted[i];
            emit BlacklistUpdated(users[i], blacklisted[i]);
        }
    }

    // Reward configuration (with limits to prevent abuse)
