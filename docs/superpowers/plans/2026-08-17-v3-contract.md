...
> **⚠️ Executed, and partly reversed. Do not copy from this plan.** Tasks 6–8
> build the creator bond; it was implemented and then **removed in full** —
> it widened what the hot resolver key could reach and was defeated for one
> cent. The plan is kept as the record of what was done, not as a description
> of the contract. Its bond sections also repeat a claim that is **false** —
> *"a market that pays out necessarily had stake on both sides"* — including
> inside quoted contract comments. Both are explained in §5.3 of
> [the spec](../specs/2026-08-17-v3-market-rules-design.md), which is the
> binding authority.
...
- **Test counts in this plan are for the V3 file alone.** `npx hardhat test`
  runs every file in `test/` — the measured baseline on this branch is
  **50 passing, 13 pending** (V3 17, USDC dual-pool 33, and 13 skipped in the
  legacy `PredictionMarket.test.js`). So:
  - to check a task's own count, run
    `npx hardhat test test/PredictionMarket_V3.test.js` and compare with the
    number the step names;
  - to check for regressions, run `npx hardhat test` and confirm the total is
    `33 + <V3 count>` passing with 13 pending, and that nothing previously
    passing now fails.
- **Never edit `test/PredictionMarket_USDC_DualPool.test.js` or
  `test/PredictionMarket.test.js`.** They cover contracts this plan does not
  touch, and their counts are the regression baseline.
