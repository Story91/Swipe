...
/**
 * Deploys PredictionMarket_V4 and applies the launch configuration.
 *
 ...
 *
 *   V4_ADDRESS=0x... npx hardhat run scripts/deploy_v4.js --network <net>
 *
...
async function main() {
  ...
  let market;
  let address = process.env.V4_ADDRESS;
  if (address) {
    ...
    console.log(`PredictionMarket_V4 deployed at ${address}`);
    console.log(`  if anything below fails, resume with V4_ADDRESS=${address}`);
  }
  ...
  // The whole reason V4 exists. REGISTRAR_ADDRESS is the key the server holds
  // to open markets people propose; it can do that and nothing else. Optional,
  // because a deploy is still valid without it: the owner can register by hand,
  // and the role can be granted later with setRegistrar.
  const registrar = process.env.REGISTRAR_ADDRESS;
  if (registrar) {
    if (!/^0x[0-9a-fA-F]{40}$/.test(registrar)) {
      throw new Error(`REGISTRAR_ADDRESS is not an address: ${registrar}`);
    }
    if (registrar.toLowerCase() === deployer.address.toLowerCase()) {
      throw new Error(
        "REGISTRAR_ADDRESS is the deployer. The point of the role is that the " +
          "key on the server cannot resolve markets, and the deployer can."
      );
    }
    await sendConfig("setRegistrar", () => market.setRegistrar(registrar, true), deployer);
  } else {
    console.log("  setRegistrar skipped, REGISTRAR_ADDRESS not set");
  }
  ...
  if (registrar) {
    // Read the role back for the same reason the fees are read back. And read
    // the resolver flag too: if the registrar somehow also holds that, the
    // split has bought nothing and the deploy should say so rather than
    // reporting success.
    const [isRegistrar, isResolver] = await Promise.all([
      market.registrars(registrar),
      market.resolvers(registrar),
    ]);
    console.log(`  registrar         ${registrar} ${isRegistrar ? "granted" : "NOT GRANTED"}`);
    if (!isRegistrar) wrong.push(`registrar ${registrar} was not granted`);
    if (isResolver) wrong.push(`registrar ${registrar} is also a resolver, which defeats the split`);
  }
  if (wrong.length) {
    throw new Error(
      `Configuration did not stick: ${wrong.join(", ")}. ` +
        `Resume with V4_ADDRESS=${address} rather than redeploying.`
    );
  }
