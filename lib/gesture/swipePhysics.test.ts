...
describe('geometry', () => {
  it('derives the dock from card width, not a fixed pixel value', () => {
    // 330px desktop card and 260px small-phone card must both dock on-card.
    expect(geometry(330, 400).dock).toBeCloseTo(151.8, 6);
    expect(geometry(280, 380).dock).toBeCloseTo(128.8, 6);
    expect(geometry(260, 340).dock).toBeCloseTo(119.6, 6);
  });
...
describe('decideArmedRelease', () => {
  ...
  it('settles back onto the dock when the card has not moved', () => {
    expect(decideArmedRelease(dock, 0, 1, dock).kind).toBe('settle');
  });

  it('commits when pushed far enough further out', () => {
    // extra = 110 * 0.55 = 60.5
    expect(decideArmedRelease(dock + 61, 0, 1, dock).kind).toBe('commit');
    expect(decideArmedRelease(dock + 60, 0, 1, dock).kind).toBe('settle');
  });

  it('cancels when pulled far enough back toward centre', () => {
    expect(decideArmedRelease(dock - 61, 0, 1, dock).kind).toBe('cancel');
    expect(decideArmedRelease(dock - 60, 0, 1, dock).kind).toBe('settle');
  });
  ...
  it('lets a flick commit without covering the full distance', () => {
    expect(decideArmedRelease(dock + 5, 500, 1, dock).kind).toBe('commit');
  });

  it('does not commit a card being flicked back inward', () => {
    expect(decideArmedRelease(dock + 61, -1000, 1, dock).kind).toBe('cancel');
  });
});
