...
export async function POST(
  ...
  try {
    ...
    console.log(`📸 Generating fresh OG image for prediction: ${id} (${chain})`);
    ...
    const imgbbResponse = await uploadBufferToImgBB(imageBuffer, `og-${id}-${timestamp}.png`);
    ...
    console.log(`💾 Saved ogImageUrl to Redis for prediction: ${id}`);
    ...
    console.error('Error generating/uploading OG image:', error);
    return NextResponse.json(
